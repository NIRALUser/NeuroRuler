...
CircumferenceWindow.enable_and_disable_elements() is bugged and not enabling the Export CSV option like it should.
...
Lastly, the two classes could (and maybe should) be separated into different files.

But the need for global variables makes this challenging."""
...
class MainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.action_export_jpg.triggered.connect(
            lambda: self.export_slice_as_img('jpg'))
        self.action_export_bmp.triggered.connect(
            lambda: self.export_slice_as_img('bmp'))
        self.action_export_ppm.triggered.connect(
            lambda: self.export_slice_as_img('ppm'))
        self.action_export_xbm.triggered.connect(
            lambda: self.export_slice_as_img('xbm'))
        self.action_export_xpm.triggered.connect(
            lambda: self.export_slice_as_img('xpm'))
    ...
    def render_curr_slice(self) -> None:
        ...
        Almost the same as `CircumferenceWindow.render_curr_slice()`.

        Doesn't do contouring and doesn't set contour pixels to red.

        Also uses gray2qimage instead of array2qimage.
        ...
        # TODO: Probably should truncate the path
        self.image.setStatusTip(str(
    ...
    def export_slice_as_img(self, extension: str):
        ...
        path: str = str(
            settings.IMG_DIR / f'{file_name}_{theta_x}_{theta_y}_{theta_z}_{slice_z}.{extension}')
...
class CircumferenceWindow(QMainWindow):
    ...
    def render_curr_slice(self):
        """Called after clicking Apply (i.e., switch to CircumferenceWindow).

        Mostly the same as `MainWindow.render_curr_slice()` but sets the pixels on the contour to red.

        Resamples the currently selected MRIImage using rotation and slice settings, then renders the resulting slice in the GUI.

        Also sets text for `image_num_label` and file path in the status bar tooltip.

        `MainWindow.render_curr_slice()` has some comments that are omitted here."""
        curr_mri_image: MRIImage = globs.IMAGE_LIST[globs.IMAGE_LIST.index]
        index: int = globs.IMAGE_LIST.index
        rotated_slice: sitk.Image = curr_mri_image.resample()

        # Don't retranspose since QImage treats w and h inversely compared to np
        # This keeps the shape of the QImage and np array the same
        binary_contour_slice: np.ndarray = imgproc.contour(
            rotated_slice, False)

        slice_np: np.ndarray = sitk.GetArrayFromImage(rotated_slice)
        # Retranspose for view since GetArrayFromImage returns the transpose
        slice_np = np.ndarray.transpose(slice_np)
    ...
    def export_slice_as_img(self, extension: str):
        ...
        Almost the same as MainWindow.export_slice_as_img.

        Exports contoured image currently displayed in GUI to settings.IMG_DIR.

        Filename has format {file_name}_contoured_{theta_x}_{theta_y}_{theta_z}_{slice}.{extension}

        Supported formats in this function are the ones supported by QPixmap, namely BMP, JPG, JPEG, PNG, PPM, XBM, XPM."""

        with open('test.txt', 'w') as f:
            f.write('circumference window export slice called')
        curr_mri_image: MRIImage = globs.IMAGE_LIST[globs.IMAGE_LIST.index]
        # This gets the last part of the file name, i.e. what's after the last slash
        # Also add 1 for 1-indexing
        file_name = globs.IMAGE_LIST.index + \
