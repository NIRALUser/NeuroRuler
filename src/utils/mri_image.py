...
More information about the Python class definition conventions followed in this file:
https://stackoverflow.com/questions/2627002/whats-the-pythonic-way-to-use-getters-and-setters.

And https://realpython.com/python-property/.

See https://peps.python.org/pep-0258/#attribute-docstrings about how to write docstrings
...
from src.utils.constants import (
    degrees_to_radians,
    deprecated,
    NIFTI_METADATA_UNITS_KEY,
    NIFTI_METADATA_UNITS_VALUE_TO_PHYSICAL_UNITS,
)
...
class MRIImage:
    ...
    Attributes are the following (for now):
        path: Path
        base_img: sitk.Image, and do not mutate!
        center: tuple
        theta_x, theta_y, theta_z, slice_num: int
        metadata: dict[str, str]

    The most important method is resample(), which returns a 2D rotated slice."""
    ...
    def __init__(
        ...
        theta_y: int = 0,
        theta_z: int = 0,
        slice_num: Union[int, None] = None,
    ) -> None:
        """:param path:
        :type path: Path
        :param theta_x: default 0
        :type theta_x: int
        :param theta_y: default 0
        :type theta_y: int
        :param theta_z: default 0
        :type theta_z: int
        :param slice_num: default None. If there isn't a slice_num arg, then slice_num will be `int((self._base_img.GetSize()[2] - 1) / 2)`
        :type slice_num: int or None"""
        self._path: Path = path
        """Path to MRI image. Get."""
        READER.SetFileName(str(path))
        self._base_img: sitk.Image = READER.Execute()
        ...
        Not necessary to encapsulate since it can be generated from `base_img`, but this prevents
        recalculation."""
        self._theta_x: int = theta_x
        """X rotation value in degrees. Get, set."""
        self._theta_y: int = theta_y
        """Y rotation value in degrees. Get, set."""
        self._theta_z: int = theta_z
        """Z rotation value in degrees. Get, set."""
        self._slice_num: int = (
            int((self._base_img.GetSize()[2] - 1) / 2)
    ...
    def __repr__(self) -> str:
        """Prints in the form MRIImage({self._path}, {self._theta_x}, {self._theta_y}, {self._theta_z}, {self._slice_num})

        Ignores `base_img` since it's determined by `path`.
    ...
    def get_physical_units(self) -> Union[str, None]:
        ...
        :return: units or None if no units detected
        :rtype: str or None"""
        if NIFTI_METADATA_UNITS_KEY in self._metadata:
            return NIFTI_METADATA_UNITS_VALUE_TO_PHYSICAL_UNITS[
                self._metadata[NIFTI_METADATA_UNITS_KEY]
            ]
        return None
    ...
    def deepcopy(self) -> "MRIImage":
        ...
        Deprecated because duplicate paths should not be possible. Do NOT use this outside of unit tests
    ...
    def resample(self) -> sitk.Image:
        """Returns the rotated 2D slice that's the result of
        resampling with this instance's rotation and slice values.

        The slice is also smoothed if `settings.SMOOTH_BEFORE_RENDERING` is True.

        :return: Rotated 2D slice
        :rtype: sitk.Image"""
    ...
    def resample_hardcoded(
        ...
        Used in test functions. Should not be used in actual program.

        The slice is also smoothed if `settings.SMOOTH_BEFORE_RENDERING` is True.

        Internally, calls `resample()`. Does not mutate the instance's attributes (i.e.,
...
class MRIImageList(_collections_abc.MutableSequence):
    """Wraps a `list[MRIImage]` and a single `index`, the index of the current `MRIImage` in the GUI (maybe
    batch processing later).

    Provides methods for moving the index by 1 (next and previous) and modifying the list.
    ...
    Duplicate paths are not allowed. There's probably a better way to do this but yeah.

    The methods involving `_paths` are not unit tested but work in the GUI.

    Mostly doesn't handle edge cases such as removing from empty list or list of length 1
    (no image to display) since that is handled in the GUI."""

    def __init__(
        self, init_list: Union[list[MRIImage], "MRIImageList", None] = None
    ) -> None:
        """Populates this instance's `images` list with an `init_list` that defaults to None (i.e., no arg).
        Also populates the `paths` set using the paths of the `init_list`.

        :param init_list:
        :type init_list: list[MRIImage], MRIImageList, or None"""
    ...
    def __repr__(self) -> str:
        ...
        Looks like [MRIImage(...), MRIImage(...), ... , MRIImage(...)]

        where the ... comes from the `str` representation of an MRIImage."""
