...
def rotate_and_export_slice(
    ...
    **kwargs
    ...
    euler_3d_transform = sitk.Euler3DTransform(
        sitk_3Dimg.TransformContinuousIndexToPhysicalPoint([(sz - 1 / 2.0) for sz in sitk_3Dimg.GetSize()]))
    ...
    slice = rotated_img[:, :, slice_num] if axis == 'z' else rotated_img[:, slice_num,
                                                             :] if axis == 'y' else rotated_img[slice_num, :, :]
